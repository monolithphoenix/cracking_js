...
function func() {
  if (i <= imagesList.length) {
    gallery.innerHTML += `<img src=${imagesList[i]}></img>`
    if ((i + 1) < imagesList.length) {
      gallery.innerHTML += `<img src=${imagesList[i + 1]}></img>`
    }
    if ((i + 2) < imagesList.length) {
      gallery.innerHTML += `<img src=${imagesList[i + 2]}></img>`
    }
    i += 3;
  }
}
